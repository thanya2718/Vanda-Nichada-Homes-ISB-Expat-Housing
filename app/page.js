...
const STATUS_STORAGE_KEY = "vanda-property-status";

function getHomeStatus(home, statusOverrides = {}) {
  return statusOverrides[home.id] || home.status || DEFAULT_PROPERTY_STATUS[home.id] || "available";
}
...
const propertyImageSets = {
  ...
  NT786: [
    ...
    "images/NT786/NT786_3.jpg",
    "images/NT786/NT786_4.jpg",
    "images/NT786/NT786_5.jpg",
    "images/NT786/NT786_6.jpg",
    "images/NT786/NT786_7.jpg",
    "images/NT786/NT786_8.jpg",
...
/*===================================เริ่มข้อมูลบ้าน (ภาษาไทย) ============================================*/
    ...
    homes: [
      ...
      {
        id: "NT853",
        title: "Nichada Thani (NT853)",
        price: "ค่าเช่า: 1xx,xxx บาท/เดือน", 
        tag: "Private",
        status: "available",
        images: propertyImageSets.NT853,
        detail: "บ้านเดี่ยวสนามกว้าง บรรยากาศเงียบสงบ เหมาะสำหรับเด็กและสัตว์เลี้ยง",
...
function StatusBadge({ status, lang }) {
  const statusInfo = PROPERTY_STATUS[status] || PROPERTY_STATUS.available;

  return (
    <span className={`rounded-full px-3 py-1 text-xs font-bold uppercase tracking-widest shadow ${statusInfo.badgeClass}`}>
      {statusInfo[lang] || statusInfo.en}
    </span>
  );
}

function AdminStatusPanel({ homes, lang, statusOverrides, onStatusChange }) {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div className="fixed bottom-4 right-4 z-[60] w-[calc(100%-2rem)] max-w-md rounded-3xl border border-stone-200 bg-white p-4 text-stone-900 shadow-2xl">
      <button
        type="button"
        onClick={() => setIsOpen(!isOpen)}
        className="flex w-full items-center justify-between rounded-2xl bg-stone-900 px-4 py-3 text-left text-sm font-bold text-white"
      >
        <span>{lang === "th" ? "Admin: เปลี่ยนสถานะบ้าน" : "Admin: Property Status"}</span>
        <span>{isOpen ? "−" : "+"}</span>
      </button>

      {isOpen && (
        <div className="mt-4 max-h-[60vh] overflow-y-auto pr-1">
          <p className="mb-3 rounded-2xl bg-amber-50 px-3 py-2 text-xs leading-5 text-amber-800">
            {lang === "th"
              ? "Phase 1: การเปลี่ยนสถานะจะบันทึกในเครื่องนี้ก่อน หากต้องการให้ทุกคนเห็นเหมือนกัน ต้องเชื่อม Google Sheet / Firebase เพิ่มเติม"
              : "Phase 1: Status changes are saved on this device. To sync publicly for all visitors, connect Google Sheet / Firebase later."}
          </p>
          <div className="grid gap-3">
            {homes.map((home) => {
              const currentStatus = getHomeStatus(home, statusOverrides);

              return (
                <div key={home.id} className="rounded-2xl border border-stone-200 bg-stone-50 p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-bold">{home.title}</p>
                      <p className="text-xs text-stone-500">ID: {home.id}</p>
                    </div>
                    <StatusBadge status={currentStatus} lang={lang} />
                  </div>
                  <select
                    value={currentStatus}
                    onChange={(event) => onStatusChange(home.id, event.target.value)}
                    className="mt-3 w-full rounded-xl border border-stone-300 bg-white px-3 py-2 text-sm outline-none focus:border-stone-900"
                  >
                    <option value="available">{lang === "th" ? "ยังว่างอยู่" : "Available"}</option>
                    <option value="rented">{lang === "th" ? "ถูกเช่าแล้ว" : "Rented"}</option>
                  </select>
                </div>
              );
            })}
...
function PropertyCard({ home, contact, primaryCta, lang, status }) {
...
export default function VandaNichadaWebsite() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Cannot load property status", error);
    }
  }, []);

  function handleStatusChange(propertyId, nextStatus) {
    setStatusOverrides((currentStatus) => {
      const updatedStatus = {
        ...currentStatus,
      ...
      if (typeof window !== "undefined") {
        window.localStorage.setItem(STATUS_STORAGE_KEY, JSON.stringify(updatedStatus));
      }
            ...
            <div className="mt-10 grid gap-6 lg:grid-cols-3">
              {t.homes.map((home) => (
                <PropertyCard
                  ...
                  lang={lang}
                  status={getHomeStatus(home, statusOverrides)}
                />
              ))}
            </div>
          </div>
