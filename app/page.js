...
const content = {
  th: {
    ...
    heroTitle: "บ้านเช่าพรีเมียมในนิชดาธานี ใกล้ ISB สำหรับครอบครัวนานาชาติ",
    ...
    homesSubtitle: "สามารถเปลี่ยนรูปและรายละเอียดเป็นบ้านจริงจาก Facebook Page หรือรายการล่าสุดได้",
    homes: [
      { title: "Modern Pool Home near ISB", tag: "Premium", detail: "บ้านสไตล์โมเดิร์น พร้อมสระว่ายน้ำส่วนตัว สวนกว้าง และพื้นที่ครอบครัว", specs: ["4–5 ห้องนอน", "สระว่ายน้ำส่วนตัว", "ใกล้ ISB", "เหมาะกับครอบครัว"] },
          ...
          <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:py-24">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.75 }} className="relative">
              <div className="aspect-[4/5] overflow-hidden rounded-[2rem] bg-stone-200 shadow-2xl">
                <img src="https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?auto=format&fit=crop&w=1200&q=80" alt="Premium family home near ISB" className="h-full w-full object-cover" />
              </div>
        ...
        <section id="homes" className="bg-white py-20">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mt-10 grid gap-6 lg:grid-cols-3">
              {t.homes.map((home, index) => (
                <article key={home.title} className="overflow-hidden rounded-[2rem] border border-stone-200 bg-stone-50 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
                  <div className="relative aspect-[4/3] overflow-hidden bg-stone-200">
                    <img src={homeImages[index]} alt={home.title} className="h-full w-full object-cover transition duration-500 hover:scale-105" />
                    <span className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-bold uppercase tracking-widest text-stone-700">{home.tag}</span>
                  </div>
                  <div className="p-6">
                    ...
                    <div className="mt-5 grid gap-2">
                      {home.specs.map((spec) => (
                        <div key={spec} className="flex items-center gap-2 text-sm text-stone-700"><Icon name="check" className="h-4 w-4" /> {spec}</div>
                      ))}
                    </div>
                  </div>
                </article>
